...
When Neo4j is newly installed, the default user *neo4j* has the default password *neo4j* . The default password must be [changed from the desktop interface](https://neo4j.com/developer/neo4j-desktop/#desktop-reset-password) before access to resources will be permitted.
